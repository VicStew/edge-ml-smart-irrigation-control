...
The current firmware reads AM2301A, analog soil-moisture, DS18B20, divided voltage inputs, and tank float switches. The master also reads its own AM2301A on GPIO27 and publishes the result as master telemetry. Sensor nodes use panel voltage as a sunlight-level reading and separately monitor their 4.2 V battery, while section and master nodes monitor their own battery voltage. Section nodes add a ZJ-G1 water-flow reading. Sensor nodes send measurements through section controllers, and the master applies a direct soil-moisture irrigation algorithm before sending actuator commands back. The master monitors low and high tank levels and controls a tank-fill pump using the ThingsBoard `manual_control` and `pump_state` shared attributes. Embedded AI inference is disabled for now; the rainfall model folders remain available for research and future integration.
...
| `master_esp/` | Master ESP32 firmware. Receives sensor packets, monitors tank levels, publishes telemetry, controls the fill pump, and sends irrigation commands to section nodes. |
...
| `sensor_esp/` | Sensor node firmware for AM2301A ambient readings, analog soil moisture, DS18B20 soil temperature, solar-panel voltage/sunlight level, and filtered battery voltage. |
...
Sensor and section nodes transmit ambient temperature/humidity, raw and calibrated soil moisture, soil temperature, and their voltage-divider measurements. Sensor nodes expose solar-panel voltage as `solar_panel_voltage_v`, `sunlight_level`, and `sunlight_level_v`, plus filtered battery voltage and a master-calculated `battery_percentage`; section and master batteries also receive percentage estimates. Section nodes additionally transmit flow rate and accumulated water volume. The master caches readings per section and node, selects the driest valid reading per section, and scales a bounded irrigation duration from the measured soil-moisture deficit. Section nodes open and close each motorized valve with interlocked relays and non-blocking 24-second travel sequences. The master debounces low/high tank float switches. Its `manual_control` shared attribute selects manual or autonomous tank filling, `pump_state` supplies the relay command in manual mode, and master telemetry reports the relay state actually applied.
