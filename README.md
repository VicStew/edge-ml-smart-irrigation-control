...
The current firmware reads AM2301A, analog soil-moisture, DS18B20, divided voltage inputs, and tank float switches. Sensor nodes use panel voltage as a sunlight-level reading and separately monitor their 4.2 V battery, while section and master nodes monitor their own battery voltage. Section nodes add a ZJ-G1 water-flow reading. Sensor nodes send measurements through section controllers, and the master applies a direct soil-moisture irrigation algorithm before sending actuator commands back. The master monitors low and high tank levels and controls a tank-fill pump from a ThingsBoard `pump_control` shared attribute. Embedded AI inference is disabled for now; the rainfall model folders remain available for research and future integration.
